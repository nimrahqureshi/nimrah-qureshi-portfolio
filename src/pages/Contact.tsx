        ...
        <div className="lg:col-span-5 space-y-6 text-left">
          ...
          <div className="flex flex-wrap gap-2 pt-2">
            {[
              ...
              { icon: Globe, url: 'https://github.com/nimrahqureshi', label: 'GitHub' },
              { icon: MessageSquare, url: 'mailto:nimrahqureshi013@gmail.com', label: 'Mail' }
            ].map((soc, i) => (
              <a 
                key={i} 
                href={soc.url} 
        ...
        <div className="lg:col-span-7">
          <form onSubmit={handleSubmit} className="premium-card p-6 sm:p-8 space-y-4 text-left bg-[#1E2023]/80">
            ...
            <button
              type="submit"
              disabled={sending}
              className="w-full premium-btn-primary py-3.5 text-xs font-mono uppercase tracking-wider flex items-center justify-center gap-2 disabled:opacity-50 mt-2 bg-gradient-to-r from-purple-600 to-indigo-600 rounded-xl text-white transition-all hover:opacity-90 active:scale-[0.99]"
            >
              {sending ? (
                <>
                ...
                <>
                  <Send className="w-4 h-4" /> Dispatch Connection Request
                </>
              )}
            </button>
          </form>
